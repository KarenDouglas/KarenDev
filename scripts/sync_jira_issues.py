...
github_response = requests.get(GITHUB_API_URL, headers={
    "Authorization": f"token {GITHUB_TOKEN}",
    "Content-Type": "application/json"
})

if github_response.status_code != 200:
    print("Failed to fetch issues from GitHub. Exiting...")
    exit(1)

existing_github_issues = github_response.json()

# Create a list of existing GitHub issue titles
existing_titles = [issue['title'] for issue in existing_github_issues]

# Loop through JIRA issues and create GitHub issues if they don't exist
for issue in jira_issues['issues']:
    title = issue['fields']['summary']
    description = issue['fields'].get('description', "No description provided.")
    ...
    if title not in existing_titles:
        ...
        print(f"Skipped creating duplicate issue: {title}")
